...
-- CreateTable
CREATE TABLE IF NOT EXISTS "IssueDetection" (
    "id" TEXT NOT NULL,
...
-- CreateIndex
CREATE UNIQUE INDEX IF NOT EXISTS "Farmer_email_key" ON "Farmer"("email");

-- AddForeignKey
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'Field_farmerId_fkey') THEN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'AnalysisReport_fieldId_fkey') THEN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'IssueDetection_fieldId_fkey') THEN
